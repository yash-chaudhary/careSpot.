...
# prediction Azure ML API route
@app.route('/prediction', methods=['POST'])
def predictions(): 
    
    # Post request data from frontend
    data = request.json

    # Encoding request data 
    body = str.encode(json.dumps(data))

    # Use environemnt variables for API call
    url = os.environ['ENDPOINT']
    api_key = os.environ['KEY']
    headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key)}

    # Making request to Azure ML REST endpoint
    req = urllib.request.Request(url, body, headers)

    try:
        # Decode response and send ML prediction to frontend
